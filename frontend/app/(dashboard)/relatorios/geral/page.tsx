...
export default function RelatorioGeralPage() {
  ...
  const totaisStatus = useMemo(() => {
    ...
    const parciais = relatorioGeral.resumos.reduce(
      (total, resumo) => total + resumo.totalParciais,
      0,
    );

    const pendentes = relatorioGeral.resumos.reduce(
      (total, resumo) => total + resumo.totalPendentes,
      0,
    );

    const faltas = relatorioGeral.resumos.reduce(
      (total, resumo) => total + resumo.totalFaltas,
      0,
    );

    /*
     * Como o mock atual não possui uma propriedade
     * totalCompletos, usamos como referência os funcionários
     * que não apresentam falta, parcial ou pendência.
     */
    const completos = relatorioGeral.resumos.filter(
      (resumo) =>
    ...
    () => [
      {
        header: "Funcionário",
        render: (resumo) => {
          const iniciais = resumo.funcionario.nome
            .split(" ")
            .filter(Boolean)
            .slice(0, 2)
            .map((nome) => nome[0])
  ...
  async function gerarRelatorio() {
    await buscarRelatorioGeral(mes, ano);
  }
        ...
        <>
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <DashboardMotion delay={0.15} className="h-full">
              <StatCard
                ...
                value={formatarMinutosParaHoras(
                  relatorioGeral.totalHorasTrabalhadasMin,
                )}
            ...
            <DashboardMotion delay={0.2} className="h-full">
              <StatCard
                title="Horas extras"
                value={formatarMinutosParaHoras(
                  relatorioGeral.totalHorasExtrasMin,
                )}
          ...
          <DashboardMotion delay={0.28}>
            <div className="grid gap-4 xl:grid-cols-[1.35fr_0.65fr]">
              ...
              >
                <StatusPieChart
                  completos={totaisStatus.completos}
                  parciais={totaisStatus.parciais}
                  faltas={totaisStatus.faltas}
                  pendentes={totaisStatus.pendentes}
                />
              </SectionCard>
            </div>
          </DashboardMotion>
