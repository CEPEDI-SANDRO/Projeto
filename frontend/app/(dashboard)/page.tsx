...
export default function DashboardPage() {
  ...
  // Carrega os dados da dashboard do backend na montagem do componente
  useEffect(() => {
    carregarDashboard();
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      {loading && dashboard.resumo.totalFuncionarios === 0 ? (
        <div className="flex h-64 flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-slate-200 bg-white">
          <RefreshCw className="h-6 w-6 animate-spin text-slate-400" />
          <p className="text-sm font-medium text-slate-500">Carregando dados do painel...</p>
        </div>
      ) : (
        <>
